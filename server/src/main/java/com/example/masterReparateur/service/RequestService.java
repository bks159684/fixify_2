...
public class RequestService {
        ...
        public RequestResponse getNewRequestDetail(Long id) {
                Request request = requestRepo.findById(id).orElseThrow(
                                () -> new MasterException("Cette demande est deja regle", HttpStatus.NOT_FOUND));
                return mapToDto(request);
        }

        private RequestResponse mapToDto(Request request) {
                List<Repairer> repairers = repairerRepo.getReapairersByModelAndProblemTypeAndCity(
        ...
        public void changeRepairer(Long id, Long reqId, Long oldId) {
                ...
                Reparation reparation = reparationRepo.findByModelIdAndProblemIdAndRepairerUsername(
                                request.getModel().getId(), request.getProblem().getId(), newRepairer.getUsername())
                                .get();
        ...
        private void sendEmails(String clientEmail, String repairerEmail, Reservation reservation) {
                List<Livreur> livreurs = livreurRepo
                                .findLivreursWithNoStatusZeroDeliveries(reservation.getClient().getCity());
                ...
                for (Admin supportTechnique : supportTechniques) {
                        mailService.sendMail(
                                        new NotificationEmail("Nouvelle reservation est met en place",
                                                        supportTechnique.getEmail(),
                ...
                mailService.sendMail(new NotificationEmail("Votre reservation est affecte  a un autre reparateur ",
                                clientEmail,
