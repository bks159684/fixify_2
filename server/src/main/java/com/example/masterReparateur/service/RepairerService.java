...
public class RepairerService {
    ...
    public RepairerDto updateRepairer(Long id, RepairerDto repairerDto) {
        Repairer existingRepairer = repairerRepo.findById(id).orElseThrow(() -> new EntityNotFoundException("Repairer not found"));
    ...
    public List<RepairerProfileDto> getQualifiedRepairers(SearchDto searchDto, int page, int size) {
        ...
        List<Repairer> repairers = repairerRepo
                .getReapairersByModelAndProblemTypeAndCity(searchDto.getModelId(), searchDto.getProblemId(),
                        searchDto.getCity(), PageRequest.of(page - 1, size))
                .stream()
                .collect(Collectors.toList());
    ...
    private List<Repairer> parseExcelFile(MultipartFile file) throws IOException {
        ...
        int usernameCol = -1, firstNameCol = -1, lastNameCol = -1, genderCol = -1,
            addressCol = -1, phoneCol = -1, cityCol = -1, emailCol = -1, passwordCol = -1,
            cinCol = -1, ribCol = -1;
    ...
    private boolean isUnique(Repairer data) {
        return !existingUsernames.contains(data.getUsername())
            && !existingCins.contains(data.getCin())
            && !existingEmails.contains(data.getEmail())
            && !existingRibs.contains(data.getRib());
    }
    ...
    public List<RepairerDto> getTopRepairers() {
        ...
        return repairerRepo.findTopRapairers().stream()
                .map(this::mapToRepairerDto)
                .collect(Collectors.toList());
    ...
    public List<RepairerDto> listFav(String client) {
        return repairerFavorisRepo.findAllByClientUsername(client).stream()
                .sorted(Comparator.comparingLong(RepairerFavoris::getId).reversed())
                .map(this::mapFavorisToRepairerDto)
                .collect(Collectors.toList());
